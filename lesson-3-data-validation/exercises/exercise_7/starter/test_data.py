...
def data():
    ...
    local_path = run.use_artifact("exercise_5/preprocessed.csv:latest").file()
...
def test_class_names(data: pd.DataFrame):
...
def test_column_ranges(data: pd.DataFrame):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        ...
        assert data[col_name].dropna().between(minimum, maximum, inclusive='both').all(), (
            f'{col_name} failed, instead min={data[col_name].min()} and max={data[col_name].max()}'
        )
